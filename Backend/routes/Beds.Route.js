...
router.post("/single", async (req, res) => {
  ...
  try {
    ...
    if (bed.length === 0) {
      return res.send({ message: "Bed does not exist" });
    }
...
router.patch("/:bedId", async (req, res) => {
  ...
  try {
    ...
    if (!bed) {
      return res.status(404).send({ msg: `Bed with id ${id} does not exist` });
    }
...
router.put("/discharge", async (req, res) => {
  ...
  try {
    ...
    if (!bed) {
      return res.status(404).send({ message: `Bed does not exist` });
    }
...
router.delete("/:bedId", async (req, res) => {
  ...
  try {
    ...
    if (!bed) {
      res.status(404).send({ msg: `Bed with id ${id} does not exist` });
    }
